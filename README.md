...
reports each of the three credentials: where it was read from, its value masked
down to the type prefix and its length, and whether the API still accepts it.
Only a credential that is set costs a request, and `[in use]` marks the one the
other commands would send.
...
A service account access key reaches one project only, and every other project
refuses a good key and a bogus one alike, so it stays `unknown` until
`--project` names the project it belongs to:
